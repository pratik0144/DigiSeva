...
Digi Seva/
├── backend/
│   ├── main.py                # Flask API Entry Point (Port 5005)
...
└── frontend/                  # React Vite Client Application
    ...
    │   ├── pages/             # App Pages (Home, VoiceInteraction, Transactions, Schemes, Education, FraudDetection)
    │   ├── index.css          # Core Design System CSS tokens & Dark Mode variables
...
- **Material Design 3 Dark Mode:** Features a dynamic, premium dark theme powered by CSS variables with persistent user preferences (`localStorage`) and native toggle controls.
...
- **Dedicated Application Modules:** 
  ...
  - *Schemes*: Fully integrated hub for browsing, filtering, and discovering over 50+ government schemes with personalized AI recommendations.
  - *Education*: Comprehensive financial literacy module featuring categorized lessons and an interactive, client-side Text-to-Speech (TTS) reading engine in multiple languages.
  - *Fraud Detection*: A dedicated dashboard tracking the user's fraud risk profile, keeping a historical log of AI-intercepted scams, and providing a community reporting form.
...
- **Agentic Orchestration:** 
  ...
  - **Schemes Agent:** Recommends and explains government schemes (e.g., PM-KISAN, MGNREGA) using an offline, rule-based recommendation engine (`SchemeRecommender`) connected to a local 50+ scheme database.
...
## 📡 API Endpoints (Artha AI - Port 5005)
...
- `GET /schemes/all`: Retrieve the entire local database of government schemes.
- `POST /schemes/recommend`: Generate personalized scheme recommendations based on user profile attributes.
